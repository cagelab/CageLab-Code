...
2. Data broadacast messages sent from PTB/MATLAB (or other experiment frameworks) are stored in cogmoteGO and can be subscribed to by any number of remote machines.
...
![Main CageLab GUI](docs/Images/cagelab-gui1.png)


We use another app `Cogmore` (cross-platform GUI writen in Tauri), to visualise both the subject screen + camera video feed from the CageLab camera[^camera] and to plot the broadcast data.

![9 remote systems streamed to a control PC](docs/Images/cogmote.png)
...
![Viewing the resource usage, and task logs directly via ssh](docs/Images/cagelab-monitor.png)
...
![Alyx Web GUI Showing experimental sessions](docs/Images/Sessions.png)
